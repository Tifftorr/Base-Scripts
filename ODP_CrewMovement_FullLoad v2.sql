...
LEFT JOIN [Reference_BusinessStructure].[tClient] CL ON CL.[Client ID] = VMS.[Client ID]
LEFT JOIN [ShipMgmt_Crewing].[tCrewTravelDetails] CMM ON CMM.[Service Record ID] = SD.[Service Record ID] AND CMM.[Travel Type] = 'Onsigner'
...
WHERE SD.[Service Cancelled] = 0
	...
	--AND cast(SD.[Start Date] as date) >= '2022-01-01' 
...
LEFT JOIN [Reference_Vessel].[tVessel] VV ON VV.[Vessel ID] = SD.[VESSEL ID]
LEFT JOIN [Reference_BusinessStructure].[tClient] CL ON CL.[Client ID] = VMS.[Client ID]
LEFT JOIN [ShipMgmt_Crewing].[tCrewTravelDetails] CMM ON CMM.[Service Record ID] = SD.[Service Record ID] AND CMM.[Travel Type] = 'Offsigner'
LEFT JOIN [ShipMgmt_Crewing].[tCrewLineUps] LNP ON LNP.[Line Up ID] = SD.[Lineup ID Leaver]
WHERE SD.[Service Cancelled] = 0
	AND SD.[Previous Experience] = 0
	AND (SD.[Service Active Status ID] is null or SD.[Service Active Status ID] <> (3) )
	AND SD.[Status ID] in ('OB','OV')
	--AND cast(SD.[End Date] as date) >= /*@sDateTime*/ '2022-01-01' 
