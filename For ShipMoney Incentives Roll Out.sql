...
select * from ( 
	select a.*, b.sgt_id from ( 
		select distinct sd.CRW_ID as crw_id,
			 ...
			 --left join shipsure.[dbo].[USERID] USR on USR.USR_ID = conta.usr_id
              ...
              and           cast(sd.SET_StartDate as DATE) <= '2024-08-15' --GETDATE() 
	...
	left join (select crw_id, sgt_id from Shipsure..CRWSrvDetail sd
				...
				and sd.SET_PreviousExp = 0
				AND sd.SET_CANCELLED = 0) b on a.crw_id = b.crw_id) c
where (c.SGT_ID is null or c.SGT_ID not in ('000015', '000016')) -- removing promotions and vessel transfers
			  ...
			  /*select top 10 * from AttributeDef where tablename like '%static%'

			select top 10 *  from CRWAttributes cat 
			select top 10 * from CRWAttributetypes cp where att_desc like '%ShipMoney%'

			atg_id = 'GLAS00000008' -- static attributes
			att_id = 'STAT00000045' -- ShipMoney Pay Agreed
			att_id = 'STAT00000046'

inner join CRWAttributetypes cp on cp.att_id=cat.att_id 

select top 10 * from [dbo].[CRWContacts2]
select top 10 * from [dbo].[CRWContactReason]
where ccr_description like '%Wage%'


select top 10 * from  [dbo].[CRWContactSiteRef]
select top 10 * from [dbo].[CRWContactActions]
select top 10 * from [dbo].[CRWContactDetails]
select top 10 * from [dbo].[CRWContentManagementData] */